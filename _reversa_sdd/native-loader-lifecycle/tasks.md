...
- [ ] NLL-T-04, Recreate the two lazy public globals. Origin: `lib/src/util.dart:10`. Done when both share `_library`, `libgit2` initializes before it is returned, and `libgit2Opts` remains independently readable unless an explicit ordering rule is added. Confidence: 🟢 code; 🔴 ordering policy

## Test Tasks
- [ ] NLL-TT-01, Test bare-name and package-path desktop loads from a foreign working directory.
- [ ] NLL-TT-01A, Exercise Windows fallback when the package-local `windows` directory is absent; define whether bare-name `libssh2.dll` lookup or an explicit error is intended. Confidence: 🔴 [Codex cross-review]
...
## Pending Gaps
🔴 Assign production shutdown ownership, decide whether options require prior init, and decide whether a failed init return must abort bootstrap.
