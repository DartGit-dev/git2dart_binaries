...
| Change workflow branch/event triggers | publication trust boundary | workflow `on`, step conditions | dry-run of PR vs push behavior; external permissions review |
| Change `git2dart_binaries` public API/version | cross-repository compatibility | pubspec, exports, consumer constraint (external) | consumer compile/test in `F:\git2dart` — currently a GAP |
...
Any change to public exports, package version/SDK constraints, generated libgit2 ABI, initialization semantics, or Android TLS contract potentially impacts `F:\git2dart`. This is a high-impact **inference** until the neighboring repository supplies direct manifest, import, and test evidence.
...
5. **public package API/version ↔ external `git2dart` dependency** (unconfirmed locally).
