...
- Download and inject only the `lib/src/bindings.dart` artifact generated by the binding job in the same workflow run. The path must never be tracked or committed, and the workflow must reject any source-checkout fallback. 🟢 user-confirmed policy
- Reject expanded packages above 256 MiB and reject pub dry-run errors. 🟢
- Validation is reachable for accepted pull requests and branch pushes; publication is reachable only for an exact push to `refs/heads/main`. 🟢 current parsed workflow facts
...
| VRA-RF-08 | Assemble and validate with the authoritative same-run CI-generated `bindings.dart`, never a committed copy. | Must | The generated path is untracked; release jobs download the generating job's artifact; a tracked file, missing artifact, cross-run artifact, or source fallback blocks validation and publication. | 🟢 user-confirmed policy |
...
| Artifact provenance | The workflow-run identity links binding generation to every validation and assembly consumer, while source control excludes the generated file. | user-confirmed policy; CI artifact-origin and tracked-file gates required | 🟢 user-confirmed policy |
...
Given `lib/src/bindings.dart` is tracked or the same-run CI artifact is unavailable
When validation or release assembly starts
Then the workflow fails instead of using a source-checkout or cross-run fallback
```

## MoSCoW
Must: complete DAG, same-run CI binding injection with no tracked copy, platform tests, selected-pair `git2dart` coordination, size/dry-run gates, and event separation. Could: signed provenance. Won't assert: current CI success without run evidence. 🟢 user-confirmed coordination and binding-artifact policy; 🔴 current CI evidence
...
- The current workflow is a 14-job DAG; `publish_package` depends on five platform-test jobs plus the three-other-ABI Android matrix after producer routes. 🟢
- Eight unique passed platform-proof scopes, release inventory, five-platform OpenSSL provenance, 256 MiB size gate, disposable consumer, and pub dry-run precede event routing. 🟢 graph
- Aggregate proof validation currently does not enforce candidate identity, complete inventory semantics, linkage/version/attestation meaning, or proof hashes against downloaded payload bytes. 🟢 observed defect
- W004-W006 local CLI/AST/YAML facts prove bounded semantics, not GitHub execution, permissions, credentials, artifacts, or pub.dev acceptance. 🟢 local; 🔴 hosted/external outcome
- No current feature-005 hosted run or exact proof→payload→bundle→publication identity join was observed. 🔴
