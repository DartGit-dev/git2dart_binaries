...
- Construct generated bindings and global-option views over one handle, then delegate checked initialization, owner pins, and terminal shutdown to the managed runtime.
...
- Negative: the generated raw ABI still exposes lifecycle methods that callers can use outside the supported manager.

## Evidence

`lib/src/runtime.dart`; `pubspec.yaml`; platform CMake/podspec files; iOS `OTHER_LDFLAGS`; commits `40c398d`, `99f4f49`, `ea87cf2`.
