...
reproduction:
  ...
  rate: "1/1 native integration suite"
  suspected_triggers:
...
traceability:
  ...
  root_cause: "Global-option integration tests restored values only on their success paths, and some tests changed options without a readable incoming value."
  reproduction_tests:
    - "test/opts_bindings_integration_test.dart: mutable global option coverage"
  regression_tests:
    - "flutter test -j 1 test/opts_bindings_integration_test.dart (2026-08-23: 12 passed)"

spec_verdict: null
change_set:
  - "test/opts_bindings_integration_test.dart: restore every readable global option from teardown or finally"
  - "test/opts_bindings_integration_test.dart: remove unobservable cache-enable mutation and make cache-object ABI coverage static"
...
Root cause is confirmed and the native integration suite passes. A human specification verdict was deliberately not inferred; hosted delivery, merge, and publication remain required by the package closure policy.
...
- See `evidence/fix-verification.md` for the cleanup and validation boundary.
