...
traceability:
  ...
  root_cause: "The publish job downloaded named artifacts then checked only aggregate size and pub metadata, without a native-file inventory gate."
  reproduction_tests:
    - "test/release_inventory_workflow_test.dart: release workflow validates the complete native inventory before publish"
  regression_tests:
    - "flutter test -j 1 test/release_inventory_workflow_test.dart (2026-08-23: pass)"

spec_verdict: null
change_set:
  - ".github/workflows/build_package.yml: add fail-closed native release inventory before size and dry-run gates"
  - "test/release_inventory_workflow_test.dart: require inventory coverage for all release platforms"
...
Root cause is confirmed and the workflow regression test passes. A human specification verdict was deliberately not inferred; a hosted release-assembly run, merge, and publication remain required for closure.
